defmodule OpenTelemetryDecorator.Attributes do
  ...
  def get(bound_vars, requested_attributes, result \\ nil) do
    bound_vars = if result, do: Keyword.put(bound_vars, :result, result), else: bound_vars
    prefix = Application.get_env(:open_telemetry_decorator, :attr_prefix) || ""

    requested_attributes
    |> Enum.reduce(%{}, fn path, attributes ->
      case get_bound_var(bound_vars, path) do
        {path, value} ->
  ...
  defp as_string(term) when is_binary(term) or is_integer(term) or is_boolean(term), do: term
